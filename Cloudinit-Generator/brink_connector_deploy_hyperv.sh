...
NPEOF
    ...
    if [[ ${ARM_MODE} -eq 2 ]]; then
        ...
        NP_CONTENT=${TWO_ARM}
        NC_CONTENT='{"config": [{"interface": {"name": "eth0", "type": "wan", "ip": "'${INET_IP}'", "gateway": "'${INET_GW}'", "dns": "'${NAME_SERVERS_INET}'"}}, {"interface": {"name": "eth1", "type": "lan", "ip": "'${DC_IP}'", "gateway": "'${DC_GW}'", "dns": "'${NAME_SERVERS_DC}'", "ipv6": "'${DC_IPV6}'", "ipv6_gw": "'${DC_IPV6_GW}'", "ipv6_dns": "'${DC_IPV6_DNS}'"}}], "arm_mode": '${ARM_MODE}', "provider": "HYV"}'
    elif [[ ${ARM_MODE} -eq 1 ]]; then
        SSH_LISTEN_ADDRESS="${INET_IP%%/*}"
        NP_CONTENT=${ONE_ARM}
        NC_CONTENT='{"config": [{"interface": {"name": "eth0", "ip": "'${INET_IP}'", "gateway": "'${INET_GW}'", "dns": "'${NAME_SERVERS_INET}'", "ipv6": "'${DC_IPV6}'", "ipv6_gw": "'${DC_IPV6_GW}'", "ipv6_dns": "'${DC_IPV6_DNS}'"}}], "arm_mode": '${ARM_MODE}', "provider": "HYV"}'
    else
