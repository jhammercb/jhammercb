...
$logFile  = "C:\Users\pocuser\Desktop\" + (Get-Date -Format "yyyyMMdd_HHmmss") + "_log.txt"
...
for ($j = 0; $j -lt 48; $j++) {
    ...
    $outputAvg = "Calculated Average`t`t$timeAvg`ts`t$speedAvg`tMbps"
    Write-Host $outputAvg
    $outputAvg | Out-File -Append $logFile

    # Print SSH command
    $sshCommand = "Running SSH Command: $($sshCommands[$j])"
    Write-Host $sshCommand
    $sshCommand | Out-File -Append $logFile

    # Execute remote SSH command and ignore output
    ssh -i $sshKeyFile $sshUsername@$sshServer $sshCommands[$j] | Out-Null
}
